create database lms;
use lms;
create table author(authorID int, fname varchar(50), lname varchar(50));
insert into author
values (1,'William Dean','Howells'),
(2,'Frederic','Brown'),
(3,'Jack','London'),
(4,'Albert','Blaisdell'),
(5,'Ellis','Butler'),
...
(18,'Art');
select * from type;
create table book(bkID int, title varchar(50), pagecount int, authorID int, typeID int);
...
(25,'The Master of Ballantre',236,16,13),
...
(37,'The Loan Star Ranger',328,30,9),
...
select * from book;
create table cust(custID int, fname varchar(30), lname varchar(30), gender varchar(10), age int, sub_start_date date, sub_end_date date);
...
select * from cust;
create table transaction(transID int, custID int, bkID int, issue_date datetime, return_date datetime);
...
/*Select customers who have not borrowed any book from library*/
select custID, Fname, lname
from cust
where custID not in (select distinct custID from transaction)
/*select customer who has borrowed some book for longest period*/
SELECT c.custID, c.fname, c.lname,
MAX(DATEDIFF(t.return_date, t.issue_date)) AS max_borrow_duration
FROM cust c
